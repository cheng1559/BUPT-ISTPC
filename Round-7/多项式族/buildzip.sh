...
# ATTENTION: This script has been modified to fit the needs of the problem.
# It has a special compile flag -lgmpxx, -lgmp, which is not included in the original script.
# This is because the generator require GMP to deal with rational numbers in order to avoid 
# generating some 'looks valid' but actually invalid test cases due to floating point error.
...
ls *.cc | while read file; do
	echo "    Compiling $file..."
	assert "Compile failed: $file" g++ -std=c++20 "-I$testlib_path" -O2 -Wall -Wextra -Wfatal-errors -Wno-unused-result -o "$work_tmp/${file%.cc}" "$file" -lm -lgmpxx -lgmp
done
if [[ -f jury.c ]]; then
	echo "    Compiling jury.c..."
	assert "Compile failed: jury.c" gcc -std=c11 "-I$testlib_path" -O2 -Wall -Wextra -Wfatal-errors -Wno-unused-result -o "$work_tmp/c.jury" "jury.c" -lm -lgmpxx -lgmp
fi
if [[ -f jury.java ]]; then
